...
# MARKET DATA
# ─────────────────────────────────────────────────────────────
@st.cache_data(ttl=0, show_spinner=False)  # always fetch fresh prices
def fetch_prices(tickers: list) -> dict:
    ...
    def get_live_and_prev(ticker):
        """Get live price (1m interval) + prev close (1d interval)."""
        try:
            # Live price — 1m interval gives real-time intraday price
            live_data = yf.download(ticker, period="1d", interval="1m",
                                    auto_adjust=True, progress=False)
            live_closes = live_data["Close"].dropna()
            if live_closes.empty:
                return None, None
            live_price = float(live_closes.iloc[-1])

            # Previous close — use 5d/1d to get last completed trading day
            prev_data = yf.download(ticker, period="5d", interval="1d",
                                    auto_adjust=True, progress=False)
            prev_closes = prev_data["Close"].dropna()
            prev_close = float(prev_closes.iloc[-2]) if len(prev_closes) >= 2 else float(prev_closes.iloc[-1])

            return live_price, prev_close
        except Exception:
            return None, None

    # Fetch 52W range once using 1y daily data (shared for all)
    try:
        hist_all = yf.download(valid, period="1y", interval="1d",
    ...
    for t in valid:
        try:
            live_price, prev_close = get_live_and_prev(t)

            # Try alternates if primary fails
            if live_price is None:
                for alt in TICKER_ALTERNATES.get(t, []):
                    live_price, prev_close = get_live_and_prev(alt)
            ...
            # 52W from the batch 1y download
            try:
                if hist_all is not None:
                    c1y = (hist_all[t]["Close"] if len(valid) > 1 else hist_all["Close"]).dropna()
                    w52h = float(c1y.max()) if not c1y.empty else None
                    w52l = float(c1y.min()) if not c1y.empty else None
                else:
                    w52h, w52l = None, None
            except Exception:
                w52h, w52l = None, None
...
with st.sidebar:
    ...
    # ── Auto-load: reads the Excel from the same folder as this script ──
    # Just drop a new Excel file in the folder and click "Reload File" to refresh.
    import os, glob
    ...
    if excel_files:
        ...
        st.caption("Drop a new Excel in the same folder and click Reload to switch files.")
        if st.button("🔄 Reload File"):
            st.cache_data.clear()
            st.rerun()
        uploaded = auto_file   # pass file PATH (string) to parser
    else:
        st.warning("No Excel file found in the app folder.")
        st.markdown(
            "**Place your Nuvama Excel file in:**\n\n"
            f"`{script_dir}`"
        )
        uploaded = None
    st.divider()
