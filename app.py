...
try:
    ...
    # Auto-detect the factsheet PDF rather than hardcoding the filename
    # (Nuvama uses sequence-numbered names that change each month — e.g.
    # "AVERRAOPS1_229_PortFolioFactSheet6020UT__86_.pdf"). We look for any
    # PDF in the repo folder whose name contains "PortFolioFactSheet"
    # (Nuvama's invariant product identifier). If multiple PDFs match
    # (e.g. you uploaded a new one but forgot to delete the old one), we
    # pick the most recently modified — i.e. the one you just uploaded.
    ...
    _factsheet_candidates = (
        ...
        + _glob.glob(_os_cfg.path.join(_repo_dir, "*PortfolioFactSheet*.pdf"))  # case variants
        + _glob.glob(_os_cfg.path.join(_repo_dir, "*portfoliofactsheet*.pdf"))
    )
    # De-duplicate while preserving order (multiple patterns may match the
    ...
    if _factsheet_candidates:
        # Most recently modified file wins — most likely the newest upload
        ...
        if len(_factsheet_candidates) > 1:
            print(f"[app] Note: {len(_factsheet_candidates)} factsheet PDFs found in repo; "
                  f"using most recent. Consider deleting older ones.")
    ...
    print(f"[app] Could not load cash data: {_cash_err}")
    cash_by_client = {}
    firm_cash_total = 0
    _factsheet_pdf_path = None
