...
def fetch_prices(tickers: list, pdf_anchor_date=None) -> dict:
    ...
    for t in valid:
        try:
            ...
            # days.
            #
            # prev_close is now VERIFIED by date (see
            # _get_verified_prev_close docstring) rather than trusted by
            # raw position — this catches the real, confirmed bug where
            # Yahoo's data had a gap (a missing row for one trading day),
            # making position -2 actually 2+ trading days back instead of
            # 1, which visibly looked like two days' gains combined into
            # one number.
            ...
            if _last_bar_is_today_ist(closes.index):
                ...
                if prev_close is not None and prev_close > 0:
                    ...
                    # Gap detected or insufficient data — don't fabricate
                    # a number from an unverified reference.
