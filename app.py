...
def _fetch_live_quote(ticker: str):
    """
    Fetch live price + previous close directly from Yahoo's real-time
    quote data (fast_info), rather than deriving it from historical daily
    bars.

    WHY THIS EXISTS: The daily-bars approach (yf.download period=1y
    interval=1d) was showing wrong "Day Chg %" for hours after market
    open, self-correcting around 1pm — a symptom that appeared suddenly
    with no code change, tracing to an UNPINNED yfinance version silently
    picking up new/different upstream behaviour on a redeploy. fast_info
    pulls from Yahoo's live quote endpoint, which is generally fresher for
    "current price right now" than the historical daily-bars endpoint
    (which seems to lag settling the current day's row on Yahoo's side).

    Returns (live_price, prev_close, day_chg_pct) — any of which may be
    None if this specific field wasn't available. Returns (None, None,
    None) entirely on failure so the caller can fall back to the
    daily-bars method for that one ticker rather than showing nothing.
    """
    try:
        info = yf.Ticker(ticker).fast_info
        # fast_info attribute names have shifted across yfinance versions —
        # try the common variants defensively rather than assuming one.
        live_price = None
        ...
        prev_close = None
        for attr in ("previous_close", "previousClose", "regular_market_previous_close"):
            v = getattr(info, attr, None)
            if v is not None:
                prev_close = float(v)
                break
        ...
        return live_price, prev_close, day_chg
    except Exception:
        return None, None, None
...
def fetch_prices(tickers: list, pdf_anchor_date=None) -> dict:
    """
    ...
    # ── Live quotes via fast_info, threaded for speed across ~40 tickers ──
    # This is the PRIMARY source for live_price/prev_close/day_chg — see
    # _fetch_live_quote's docstring for why (fresher than historical daily
    # bars, which have been observed lagging for hours after market open).
    # The batched daily-bars download below remains the source for 52W
    # high/low and close-at-PDF-date, and serves as a PER-TICKER FALLBACK
    # if fast_info fails for any individual stock.
    live_quotes = {}
    try:
        import concurrent.futures
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            future_to_ticker = {executor.submit(_fetch_live_quote, t): t for t in valid}
            for future in concurrent.futures.as_completed(future_to_ticker):
                t = future_to_ticker[future]
                try:
                    live_quotes[t] = future.result()
                except Exception:
                    live_quotes[t] = (None, None, None)
    except Exception as e:
        print(f"[app] fast_info batch fetch failed: {e} — will rely on daily-bars fallback for all tickers")
        live_quotes = {}

    # Single batch download — 1 year daily data
    # This gives us: 52W range, close-at-PDF-date, and (as fallback) the
    # today's-close / prev-close pair if fast_info didn't have a ticker.
    try:
        batch = yf.download(
    ...
    for t in valid:
        try:
            ...
            # Settled (fully historical, non-today) previous close from the
            # daily-bars data — used below as a sanity-check reference,
            # since it isn't subject to the "today not ready" issue (that
            # only affects the LAST row's freshness, not earlier ones).
            _today_present = _last_bar_is_today_ist(closes.index)
            if _today_present and len(closes) >= 2:
                settled_prev_close = float(closes.iloc[-2])
            elif not _today_present and len(closes) >= 1:
                settled_prev_close = float(closes.iloc[-1])
            else:
                settled_prev_close = None

            # PRIMARY: use fast_info's live quote if we got one for this
            # ticker — fresher than deriving from historical daily bars
            # (see _fetch_live_quote docstring for why this is preferred).
            # FALLBACK: if fast_info failed for this ticker, derive from
            # daily bars with the date-check guard (see
            # _last_bar_is_today_ist docstring) so we still don't show a
            # fabricated day-change from mismatched days.
            #
            # CROSS-CHECK: fast_info's previous_close has been observed
            # wrong for a handful of thinly-traded / recently-listed names
            # (e.g. GNG Electronics, Aequs) even while last_price was
            # correct — showing e.g. -0.75% when the real move was +1%.
            # If fast_info's prev_close disagrees with the settled
            # historical close by more than 1%, that's not normal
            # rounding/adjustment noise — trust the historical value
            # instead and recompute day-change from it.
            fi_price, fi_prev, fi_day_chg = live_quotes.get(t, (None, None, None))

            if fi_price is not None:
                live_price = fi_price
                use_fi_prev = fi_prev is not None
                if fi_prev is not None and settled_prev_close is not None and settled_prev_close > 0:
                    disagreement_pct = abs(fi_prev - settled_prev_close) / settled_prev_close * 100
                    if disagreement_pct > 1.0:
                        use_fi_prev = False  # fast_info's reference looks wrong for this stock

                if use_fi_prev:
                    prev_close = fi_prev
                    day_c      = fi_day_chg
                elif settled_prev_close is not None:
                    prev_close = settled_prev_close
                    day_c      = (live_price - prev_close) / prev_close * 100 if prev_close else None
                else:
                    prev_close = live_price
                    day_c      = None
            else:
                ...
                if _today_present and len(closes) >= 2:
                    ...
                    prev_close = live_price
                    day_c      = None  # honest "unavailable", not a fabricated number
...
def fetch_nifty500(pdf_anchor_date=None):
    """
    Fetch Nifty 500 data from yfinance (^CRSLDX).
    ...
    Substituting Nifty 500 (^CRSLDX) for BSE 500 because BSE 500's
    yfinance feed has been unreliable in practice for this dashboard
    (deployment logs showed repeated "possibly delisted" errors). The
    two indices track closely in practice; users should be made aware
    of the substitution in the UI label.
    """
    try:
    ...
    try:
        ...
        # PRIMARY: fast_info live quote (see _fetch_live_quote docstring),
        # cross-checked against the settled historical close the same way
        # as fetch_prices (see that function's comments for why — this
        # catches cases where fast_info's previous_close is wrong even
        # though last_price is correct).
        fi_price, fi_prev, fi_day_chg = _fetch_live_quote("^CRSLDX")
        if fi_price is not None:
            live_price = fi_price
            use_fi_prev = fi_prev is not None
            if fi_prev is not None and settled_prev_close is not None and settled_prev_close > 0:
            ...
            if use_fi_prev:
                ...
                prev_close = settled_prev_close
                day_chg    = (live_price - prev_close) / prev_close * 100 if prev_close else None
            else:
                prev_close = live_price
                day_chg    = None
        else:
            # FALLBACK: derive from daily bars with the date-check guard
            live_price = float(closes.iloc[-1])
            if _today_present and len(closes) >= 2:
                prev_close = float(closes.iloc[-2])
                day_chg    = (live_price - prev_close) / prev_close * 100 if prev_close else 0.0
            else:
...
with st.spinner(f"Fetching live prices for {len(tickers)} stocks…"):
    price_data                   = fetch_prices(tickers, pdf_anchor_date=_pdf_as_of_date)
