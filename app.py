...
def fetch_prices(tickers: list, pdf_anchor_date=None) -> dict:
    ...
    for t in valid:
        try:
            ...
            # days. This data now comes from a per-ticker fetch (see top
            # of function) rather than a batched multi-ticker download,
            # to avoid the batching cross-contamination issue that was
            # producing wrong numbers on unrelated stocks.
            live_price = float(closes.iloc[-1])
            if _last_bar_is_today_ist(closes.index) and len(closes) >= 2:
