...
def _get_bse_scripcode_map():
    ...
    IMPORTANT: only a SUCCESSFUL (non-empty) result is cached
    permanently. A previous version cached an empty dict on ANY failure
    (including a single transient network hiccup) — since {} is not
    None, every subsequent call saw the cache as already "set" and
    returned that permanent empty result forever, never retrying. That
    meant one bad moment early in the day could silently break BSE
    pricing for the rest of the process's life. Now a failed attempt
    just leaves the cache unset, so the next call tries again.
    """
    global _bse_scripcode_map_cache
    if _bse_scripcode_map_cache:  # non-empty dict — trust the cache
    ...
    try:
        ...
        result = news_fetch._build_isin_scripcode_map(_bse_quote_client)
        if result:  # only cache genuine successes
            _bse_scripcode_map_cache = result
        return result
    except Exception as e:
...
def _fetch_bse_history(isin: str):
    ...
    if not isin:
        return None
    scripcode_map = _get_bse_scripcode_map()
    entry = scripcode_map.get(isin.strip().upper()) if scripcode_map else None
    if not entry:
        return None
    try:
        global _bse_quote_client
        if _bse_quote_client is None:
            from bse import BSE
            _bse_quote_client = BSE(download_folder="./_bse_tmp")
        res = _bse_quote_client.equityPriceVolumeT12M(entry["scripcode"])
...
if missing.any():
    ...
    # resolves in BSE's securities map, assign a synthetic "ISIN.BSE"
    ...
    with st.spinner("Resolving new stocks via BSE…"):
        _bse_map = _get_bse_scripcode_map()
        for idx, row in port_df[missing].iterrows():
            _isin = str(row.get("ISIN", "")).strip().upper()
            if _isin and _bse_map and _isin in _bse_map:
