...
with st.sidebar:
    ...
    def _find_excel_files(directory):
        return sorted(
    ...
    def _find_cloud_excels():
        return [f for f in os.listdir(".") if f.endswith((".xlsx", ".xls"))]

    script_dir = os.path.dirname(os.path.abspath(__file__))
    local_files = _find_excel_files(script_dir)
    cloud_files = _find_cloud_excels()
    all_files   = local_files if local_files else cloud_files

    # Identify Nuvama file (contains "AS ON" or is NOT the master portfolio)
    def _is_master(fname):
        n = os.path.basename(fname).upper()
        return "MASTER" in n or "MODEL" in n or "PORTFOLIO" in n

    nuvama_files = [f for f in all_files if not _is_master(f)]
    master_files = [f for f in all_files if _is_master(f)]

    uploaded      = nuvama_files[0] if nuvama_files else (all_files[0] if all_files else None)
