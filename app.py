...
def fetch_prices(tickers: list, pdf_anchor_date=None) -> dict:
    ...
    # NOTE: We deliberately do NOT call fast_info per-ticker here anymore.
    # An earlier attempt threaded 40 individual fast_info calls to get
    # fresher live quotes, but this very likely triggered Yahoo's rate
    # limiting (yfinance added explicit YfRateLimitError handling in a
    # recent release specifically because Yahoo has tightened this) —
    # causing fast_info to fail broadly and silently fall back to the
    # exact stale-price bug we were trying to fix, but for ALL stocks
    # instead of a few. One batched request for all tickers (below) is
    # far less likely to be rate-limited than 40 individual ones.
    #
    # Single batch download — 1 year daily data
    # This gives us: today's close, prev close, and 52W range all in one call
    #
    # auto_adjust=False (RAW prices) — deliberately NOT auto-adjusted.
    # Adjusted prices retroactively bake in dividend/split corrections,
    # which is right for long-term historical continuity but WRONG for a
    # simple day-over-day % change — and yfinance has had documented bugs
    # in this exact adjustment logic ("capital gains double-counting").
    # That matches this bug's symptom precisely: today's change looking
    # like yesterday's move got added on top of today's. Day Chg % should
    # reflect raw close-to-close movement, which needs none of this.
    try:
        batch = yf.download(
    ...
    for t in valid:
        try:
            ...
            # docstring for the exact bug this guards against). Otherwise
            # we show the last known price but withhold day-change rather
            # than compute it from mismatched days.
            #
            # NOTE: we tried sourcing this from fast_info per-ticker for
            # fresher data, but that meant 40 individual Yahoo requests
            # per page load — almost certainly the cause of a follow-up
            # regression (rate limiting → broad failures → same bug for
            # everyone instead of a few). Reverted to this simpler,
            # single-batched-request version, which is proven reliable
            # even though it has a real limitation: on the rare occasion
            # Yahoo's daily bar for TODAY exists but its price value is
            # still settling (not just missing), this can still show a
            # stale number for the first part of the day. That's an
            # accepted trade-off for reliability over freshness.
