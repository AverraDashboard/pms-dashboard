...
def consolidate(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Auto-detect tickers for any stock not in ISIN_TO_NSE map
    for idx, row in grp[grp["NSE_Ticker"].isna()].iterrows():
        detected = auto_detect_ticker(row["ISIN"], row["Instrument Name"])
        if detected:
            grp.at[idx, "NSE_Ticker"] = detected
