...
def fetch_prices(tickers: list, pdf_anchor_date=None) -> tuple[dict, dict]:
    """
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        futures = {executor.submit(_fetch_one_ticker_closes, t): t for t in valid}
        for future in concurrent.futures.as_completed(futures):
            t, closes = future.result()
            per_ticker_closes[t] = closes

    # ── PRIMARY live-quote source: nsetools, direct from NSE ──
    # Only for .NS tickers — nsetools is NSE-only, doesn't cover BSE-only
    # names (Timex, ASM Technologies), which fall through to the
    # yfinance-based path below entirely, unchanged.
    nse_quotes = {}
    ...
    for t in valid:
        try:
            ...
            # ── PRIMARY: override with nsetools' direct-from-NSE quote if
            # we got one for this ticker. This is NSE's own pre-computed
            # previousClose/pChange, not something we derived from
            # possibly-gappy historical daily bars — see _fetch_nse_quote
            # docstring for the full reasoning on why this is preferred. ──
    ...
    # needing to dig through Streamlit Cloud logs to know if nsetools is
    # actually working, which caused real friction earlier in this
    # project's build (a whole detour just to check an API key's status).
...
with st.spinner(f"Fetching live prices for {len(tickers)} stocks…"):
    price_data, _price_diagnostic = fetch_prices(tickers, pdf_anchor_date=_pdf_as_of_date)
    bse_ret, bse_day, bse_price   = fetch_benchmark()
    nifty500_data                 = fetch_nifty500(pdf_anchor_date=_pdf_as_of_date)

# Visible status line — see _fetch_nse_quote/fetch_prices docstrings for
# why nsetools (direct-from-NSE) is now the primary price source, with
# yfinance as fallback. This avoids needing to hunt through Streamlit
# Cloud's logs to know whether it's actually working, same reasoning as
# the earlier "Relevance filter: ON/OFF" indicator for the news system.
if _price_diagnostic["nsetools_available"]:
    _n_ok  = _price_diagnostic["nse_succeeded"]
    _n_tot = _price_diagnostic["nse_attempted"]
    if _n_tot > 0:
        _pct = _n_ok / _n_tot * 100
        if _pct >= 80:
            st.caption(f"🟢 Live prices: {_n_ok}/{_n_tot} direct from NSE, rest via fallback")
        elif _pct > 0:
            st.caption(f"🟡 Live prices: {_n_ok}/{_n_tot} direct from NSE, rest via fallback — check logs if this stays low")
        else:
            st.caption(f"🔴 Live prices: NSE direct fetch failed for all stocks, using fallback method for everything")
else:
    st.caption("🔴 nsetools not installed — using fallback price method for everything. Check requirements.txt.")
