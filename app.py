...
_bse_scripcode_map_cache: dict | None = None
_bse_quote_client = None

def _get_bse_scripcode_map():
    """Reuses news_fetch's already-proven ISIN->scripcode builder (the
    exact same lookup that's been powering BSE filings reliably all
    session) rather than rebuilding this logic from scratch."""
    global _bse_scripcode_map_cache
    if _bse_scripcode_map_cache is not None:
        return _bse_scripcode_map_cache
    if not NEWS_MODULE_AVAILABLE:
        _bse_scripcode_map_cache = {}
        return _bse_scripcode_map_cache
    try:
        from bse import BSE
        ...
        _bse_scripcode_map_cache = news_fetch._build_isin_scripcode_map(_bse_quote_client)
    except Exception as e:
        print(f"[app] Could not build BSE scripcode map for price fetching: {e}")
        _bse_scripcode_map_cache = {}
    return _bse_scripcode_map_cache
...
def _fetch_bse_quote(isin: str):
    ...
    if not isin:
        return None
    scripcode_map = _get_bse_scripcode_map()
    if not scripcode_map:
        return None
    entry = scripcode_map.get(isin.strip().upper())
    if not entry:
        return None
    try:
        global _bse_quote_client
        if _bse_quote_client is None:
            from bse import BSE
            _bse_quote_client = BSE(download_folder="./_bse_tmp")
        q = _bse_quote_client.quote(entry["scripcode"])
...
def fetch_prices(tickers: list, pdf_anchor_date=None, ticker_to_isin: dict | None = None) -> tuple[dict, dict]:
    ...
    # PER-TICKER downloads instead of one batched multi-ticker call.
    #
    # WHY: yfinance's batched download with group_by="ticker" has a
    # documented reliability issue — the SAME ticker can return DIFFERENT
    # results depending on what other tickers it's batched alongside, or
    # the order they're requested in (see ranaroussi/yfinance#626,
    # "Inconsistent results — a problem with yfinance or yahoo?"). This
    # matches our symptom precisely: wrong reference prices on unrelated
    # stocks (Orchid Pharma, GNG Electronics, FiveStar) with no common
    # cause between them — pointing at the BATCHING mechanism itself,
    # not anything about individual stocks. Fetching each ticker alone
    # avoids this cross-contamination entirely.
    #
    # Trade-off: this is slower (up to ~40 separate requests instead of
    # 1) and could theoretically risk rate-limiting — but unlike the
    # earlier failed fast_info attempt (which hit Yahoo's heavier
    # info/fast_info endpoint per stock), this uses the same lightweight
    # historical-bars endpoint as before, just not batched together.
    # Moderate threading (5 workers, not 10+) to stay well clear of
    # anything that looks like a burst of requests.
    #
    # auto_adjust=False (RAW prices) kept from the earlier fix — day
    # change should be raw close-to-close, no dividend/split adjustment
    # logic involved.
    import concurrent.futures

    def _fetch_one_ticker_closes(ticker):
        try:
            # multi_level_index=False — this yfinance version defaults to
            # ALWAYS returning a MultiIndex DataFrame, even for a single
            # ticker (confirmed in its own docstring). Without this, our
            # simple data["Close"] lookup below silently failed for every
            # single stock, which is why the last deploy showed "None"
            # across the entire table — not a data problem, a structure
            # mismatch in how we were reading yfinance's response.
            ...
            print(f"[fetch_prices] Failed to fetch {ticker}: {e}")
            return ticker, None

    per_ticker_closes = {}
    # SEQUENTIAL, not threaded — production logs showed a Segmentation
    # fault (a hard process-level crash, not a catchable Python error)
    # immediately after heavy concurrent activity across multiple
    # ThreadPoolExecutors. yfinance depends on curl_cffi (a C-extension
    # HTTP client) which has known thread-safety issues in some
    # configurations — concurrent threads all hitting it at once is the
    # most likely cause of a crash this severe. This data is secondary
    # now (BSE-direct handles live_price/day_chg for most stocks; this is
    # only for close_at_pdf + last-resort fallback), so trading speed for
    # NOT CRASHING THE WHOLE APP is the right call.
    for t in valid:
        _, closes = _fetch_one_ticker_closes(t)
    ...
    for t in valid:
        try:
            ...
            # Try alternates if empty/missing
            ...
            # ── yfinance-derived values (used as FALLBACK if nsetools
            # didn't have this ticker — e.g. BSE-only stocks, or nsetools
            # itself failed for this specific one) ──
            #
            # Latest close = today's price ONLY if the date check confirms
            # today's bar is genuinely present (see _last_bar_is_today_ist
            # docstring). Otherwise we show the last known price but
            # withhold day-change rather than compute it from mismatched
            # days.
            #
            # prev_close is VERIFIED by date (see _get_verified_prev_close
            # docstring) rather than trusted by raw position — this
            # catches the confirmed bug where Yahoo's data had a gap (a
            # missing row for one trading day), making position -2
            # actually 2+ trading days back instead of 1.
            ...
            # ── PRIORITY 2: override with nsetools' direct-from-NSE quote
            # if we got one for this ticker (secondary — BSE-direct is
            # tried first, applied further below since it takes final
            # priority over both this and the yfinance-derived fallback).
            nse_q = nse_quotes.get(t)
            if nse_q is not None and nse_q.get("live_price") is not None:
                ...
                if nse_q.get("w52h") is not None:
                    w52h = nse_q["w52h"]
                if nse_q.get("w52l") is not None:
                    w52l = nse_q["w52l"]

            # ── PRIORITY 1 (highest): override with BSE-direct quote if
            # we got one. This is BSE's own PrevClose/LTP, from the same
            # `bse` library already proven reliable in this deployment —
            # see _fetch_bse_quote docstring for the full reasoning on
            # why this now takes precedence over both nsetools (blocked
            # entirely from this deployment) and the yfinance-derived
            # fallback. ──
            ...
            # Close on PDF anchor date — find the closest available trading
            # day at or after the anchor (PDF date might be a weekend/holiday
            # in which case we take the next trading day's open price as the
            # reference; better than skipping the client entirely).
            close_at_pdf = None
            if anchor_ts is not None:
                # closes index is timezone-naive after .dropna() in most cases,
                # but be defensive — strip tz if present for safe comparison
                try:
                    idx = closes.index
                    if hasattr(idx, "tz") and idx.tz is not None:
                        ...
                        closes_naive = closes
                    # Closest trading day on or AFTER the PDF anchor
                    at_or_after = closes_naive[closes_naive.index >= anchor_ts]
                    if not at_or_after.empty:
                        close_at_pdf = float(at_or_after.iloc[0])
                    else:
                        # Anchor is in the future relative to data; fall back
                        # to the latest available close
...
def fetch_nifty500(pdf_anchor_date=None):
    """
    Fetch Nifty 500 index data — PRIMARILY from nsetools (direct from
    NSE), with yfinance (^CRSLDX) as fallback and as the source for
    close_at_pdf (historical date lookups nsetools can't do).

    Returns dict with:
        'live_price': latest close (float | None)
        'prev_close': previous trading day's close
        'day_chg_pct': today's % change (live during market hours,
                        otherwise last trading day's move)
        'close_at_pdf': close on (or just after) the PDF anchor date —
                        used for per-client "since-inception" drift math
        'move_since_pdf_pct': cumulative % move from PDF date to latest
                              close. EOD update only — we don't pretend
                              this updates intraday since BSE 500 anchors
                              are EOD-based per the user spec.

    Substituting Nifty 500 for BSE 500 because BSE 500's data feeds have
    been unreliable in practice for this dashboard. The two indices track
    closely in practice; users should be made aware of the substitution
    in the UI label.
    """
    try:
        # auto_adjust=False (RAW) — same reasoning as fetch_prices: day
        # change should be raw close-to-close, not run through
        # dividend/split adjustment logic.
        # multi_level_index=False — same structural fix as fetch_prices;
        # without it, single-ticker downloads return a MultiIndex
        # DataFrame in this yfinance version, breaking the simple
        # data["Close"] lookup silently.
        data = yf.download("^CRSLDX", period="1y", interval="1d",
                           auto_adjust=False, progress=False,
                           multi_level_index=False)
        closes = data["Close"].dropna()
        if closes.empty:
    ...
    try:
        # NOTE: fast_info-based fetching was removed here too (see
        # fetch_prices' comments for the full story) — even though this
        # is just one ticker (not 40), keeping the approach consistent
        # and simple. The index is highly liquid, so the "wrong reference
        # price for thin stocks" issue that affected GNG/Aequs is unlikely
        # to apply here anyway.
        live_price = float(closes.iloc[-1])
        if _last_bar_is_today_ist(closes.index):
            prev_close, _prev_date = _get_verified_prev_close(closes)
            if prev_close is not None and prev_close > 0:
                ...
                # Gap detected or insufficient data — same class of bug
                # as fetch_prices (see _get_verified_prev_close docstring)
                prev_close = live_price
                day_chg    = None
        else:
            # Today's bar isn't confirmed present yet — same bug class as
            # fetch_prices (see _last_bar_is_today_ist docstring). Show
            # last known price, withhold day-change rather than compute
            # it from mismatched days.
            prev_close = live_price
            day_chg    = None

        # ── PRIMARY: override with nsetools' direct-from-NSE index quote
        # if available — same reasoning as fetch_prices (NSE's own
        # pre-computed previousClose/percentChange, not derived from
        # possibly-gappy historical daily bars). ──
        if NSETOOLS_AVAILABLE:
            try:
                nse = _get_nse_client()
                if nse is not None:
                    idx_q = nse.get_index_quote("NIFTY 500")
                    if idx_q and idx_q.get("last") is not None:
                        live_price = float(idx_q["last"])
                        if idx_q.get("previousClose") is not None:
                            prev_close = float(idx_q["previousClose"])
                        if idx_q.get("percentChange") is not None:
                            day_chg = float(idx_q["percentChange"])
            except Exception as e:
                print(f"[app] nsetools NIFTY 500 index quote failed: {e}")
...
# KPI CARDS
# ─────────────────────────────────────────────────────────────
# 4 columns: Total AUM | 1-Day Return | Nifty 500 Today | 1-Day Alpha
...
if nifty500_data and nifty500_data.get("day_chg_pct") is not None:
    ...
    k3.metric("Nifty 500 Today", f"{_nifty_day:+.2f}%", delta_color="off")
    _alpha_1d = port_day_ret - _nifty_day
    k4.metric("1-Day Alpha (vs Nifty 500)", f"{_alpha_1d:+.2f}%", delta_color="off")
else:
    k3.metric("Nifty 500 Today", "—")
    k4.metric("1-Day Alpha (vs Nifty 500)", "—")
...
with st.expander("Client-wise Breakdown"):
    ...
    cg["Nifty 500 Since Inception"] = cg["Client Name"].map(lambda n: _fmt_pct(metrics_per_client[n]["nifty500_since_inception_pct"]))
    ...
    client_event = st.dataframe(
        cg[["Client Name","Stocks","Portfolio Value","% of AUM",
            "Day Chg %","Return Since Inception","Nifty 500 Since Inception"]],
        width="stretch", hide_index=True,
