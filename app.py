...
def fetch_prices(tickers: list, pdf_anchor_date=None, ticker_to_isin: dict | None = None) -> tuple[dict, dict]:
    ...
    per_ticker_closes = {}
    # max_workers reduced from 5 to 2 — production logs showed
    # YFRateLimitError firing (yfinance's own client-side rate limit
    # tripped by too many concurrent requests). This data is now only
    # needed for close_at_pdf and as a last-resort fallback (BSE-direct
    # handles live_price/day_chg for most stocks), so slower-but-reliable
    # beats fast-but-rate-limited here.
